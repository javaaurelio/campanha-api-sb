...
@Service
public class VotoCarnavalService {

	@Autowired
	private VotoRepository votoRepository;
	
	@Autowired
	private VotoCarnavalRepository votoCarnavalRepository;
...
//		dataFim = dataFim.with(LocalTime.MAX);
	...
	public void registrarVotoCarnaval(DadosCadastroDadosVoto dadosVoto, String headerNames) {
		
		List<VotoCarnaval> listaVotos = new ArrayList<VotoCarnaval>();
		
		String hash = UUID.randomUUID().toString();
		for (DadosCadastroVotos itemTela : dadosVoto.listaVoto()) {
			
			VotoCarnaval votoCarnaval = new VotoCarnaval(itemTela);
			votoCarnaval.setHash(hash);
			listaVotos.add(votoCarnaval);
		}
		try {
			votoCarnavalRepository.saveAll(listaVotos);
		} catch (Exception e) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Voto nao registrado!");
		}
	}
